...
class MatrixVC: UIViewController, UITextFieldDelegate {
    ...
    @objc fileprivate func doCalculate() {
        if let rows = Int(aTextField.text ?? "0"), let colums = Int(bTextField.text ?? "0"), rows > 0, colums > 0 {
            if rows * colums <= multiplyLimit {
                ...
                showCustomToast(message: "calculated".localized())
            } else {
                showCustomToast(message: "too_large_a_b".localized(replace: String(multiplyLimit)))
            }
        } else {
            showCustomToast(message: "enter_a_b".localized())
        }
    }
